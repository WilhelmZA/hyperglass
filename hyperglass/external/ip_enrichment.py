...
class IPInfo:
    """Result of IP lookup."""
    def __init__(self, ip: str, asn: t.Optional[int] = None, asn_name: t.Optional[str] = None, 
                 prefix: t.Optional[str] = None, country: t.Optional[str] = None,
                 is_ixp: bool = False, ixp_name: t.Optional[str] = None):
        self.ip = ip
        self.asn = asn
...
class IPEnrichmentService:
    ...
    def __init__(self):
        self.cidr_networks: t.List[t.Tuple[t.Union[IPv4Address, IPv6Address], int, int, str]] = []  # (network, prefixlen, asn, cidr_string)
        self.asn_info: t.Dict[int, t.Dict[str, str]] = {}  # asn -> {name, country}
        self.ixp_networks: t.List[t.Tuple[t.Union[IPv4Address, IPv6Address], int, str]] = []  # (network, prefixlen, ixp_name)
    ...
    async def ensure_data_loaded(self) -> bool:
        ...
        if cached_time and (datetime.now() - cached_time).total_seconds() < cache_duration:
            ...
            if cidr_data and asn_data and ixp_data:
                log.debug("Loading IP enrichment data from cache")
        ...
        try:
            log.info("Downloading fresh IP enrichment data...")
            ...
            log.info(f"IP enrichment data loaded: {len(self.cidr_networks)} CIDR entries, "
    ...
    async def _download_bgp_data(self, client: 'httpx.AsyncClient') -> None:
        ...
        log.info("Downloading BGP.tools CIDR table...")
            ...
            if line.strip():
                try:
                    ...
                    if cidr and asn:
                        ...
                        self.cidr_networks.append((network.network_address, network.prefixlen, asn, cidr))
                        cidr_count += 1
                except Exception as e:
                    continue
        
        log.info(f"Downloaded {cidr_count} CIDR entries")
        
        # Sort by prefix length (descending) for longest-match lookup
        self.cidr_networks.sort(key=lambda x: x[1], reverse=True)
        
        # Download ASN names
        log.info("Downloading BGP.tools ASN names...")
        ...
        log.info(f"Downloaded {asn_count} ASN entries with country codes")
    
    async def _download_ixp_data(self, client: 'httpx.AsyncClient') -> None:
        """Download PeeringDB IXP data."""
        log.info("Downloading PeeringDB IXP data...")
        ...
        log.info(f"Downloaded {ixp_count} IXP networks")
