...
description: Ultraglass config.yaml file reference
---
...
| `plugins`          | List of Strings |                                  | List of Ultraglass [plugins](/plugins) to load.               |
...
| :----------- | :--------------------------------------------------------------------- | :--------------------------------------------------------------- |
| `cache`      | [Caching Docs](/configuration/config/caching.mdx)                      | Customise how Ultraglass caches responses.                       |
...
| `structured` | [Structured Output Docs](/configuration/config/structured-output.mdx) | Customise how Ultraglass handles structured output from devices. |
| `web`        | [Web UI Docs](/configuration/config/web-ui.mdx)                        | Customise the look and feel of Ultraglass's web UI.              |

## Caveats

### Arista EOS

The tested Arista EOS release does not supply certain route details in JSON output for `show ip bgp regex <pattern>` or `show ip bgp community <community>`. The timestamp and attached communities are absent, so Ultraglass uses the current time and an empty community list for those responses.
