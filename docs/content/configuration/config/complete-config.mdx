...
This is a comprehensive reference for all available configuration options in Ultraglass. The configuration is written in YAML format and saved as `config.yaml` in your hyperglass configuration directory.
...
| :-------- | :--- | :------ | :---------- |
| `site_title` | String | `Ultraglass` | The name of your Ultraglass site. Used in HTML `<title>` tags and API documentation. |
...
Controls how Ultraglass caches query responses to improve performance.
...
Controls how Ultraglass processes and displays structured data from network devices.
...
<Callout type="info">
When users enter hostnames such as `example.com` instead of IP addresses, Ultraglass uses DNS-over-HTTPS to resolve them to IP addresses directly in the user's browser. This avoids sending the lookup through the application server.
</Callout>
...
| Parameter | Type | Default | Description |
| :-------- | :--- | :------ | :---------- |
| `web.text.title` | String | `Ultraglass` | Main title text. |
...
| Parameter | Type | Default | Description |
| :-------- | :--- | :------ | :---------- |
| `web.credit.enable` | Boolean | `true` | Show the Ultraglass and upstream credit in the footer. |
