...
Devices that return structured or easily parsed data can display their response in a table, or as JSON through the REST API. The following platforms currently have structured data support in Ultraglass:
...
When structured output is available, Ultraglass can check the RPKI state of each returned BGP prefix using one of two methods:
...
For external validation, Ultraglass supports two backends:
...
Ultraglass can also control which BGP communities are shown to the end user.

For devices with structured traceroute support, Ultraglass can add ASN, organisation, country, reverse DNS, and IXP details using lookups from BGP.tools.
...
<Callout type="info" emoji="ℹ️">
    **Routinator URL Format**
    ...
    The `rpki_server_url` should be the base URL of your Routinator HTTP web API endpoint. This is typically different from the RTR port (3323). Do not include the `/validity` path because Ultraglass appends it automatically.
</Callout>
