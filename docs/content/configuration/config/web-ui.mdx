...
Ultraglass provides extensive customisation options for the look and feel of the web UI.
...
-   Instructions on how to use Ultraglass
...
[OpenGraph](https://ogp.me/) controls the image, title, and description shown when you share a link. Ultraglass uses [this OpenGraph image](/opengraph.jpg) by default, but you can provide your own image.
...
Ultraglass can highlight special values in router output and provide users with additional context. For example, an organisation's BGP communities or IP address space can be highlighted with a tooltip.
...
Ultraglass can show customisable menus in the footer. Each menu can be configured with the following schema:
...
Ultraglass can show links to resources that users may find helpful. Each link can be configured with the following schema:
...
| :-------- | :-----: | :-----: | :--------------------------------------------------------------------------------------- |
| `enable`  | Boolean | `true`  | Enable or disable the display of Ultraglass and upstream credit in the footer. |
...
Most of the text in the Ultraglass UI can be overridden to suit your needs.

| Parameter                    | Type   | Default Value                         | Description                                                                                                                                                             |
| :--------------------------- | :----- | :------------------------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `web.text.title_mode`        | String | logo_only                             | See [title mode section](#title-mode)                                                                                                                                   |
| `web.text.title`             | String | Ultraglass                            | Title text.                                                                                                                                                             |
...
| `web.text.ip_error`          | String | Unable to determine IP Address        | Error displayed if Ultraglass is unable to determine the user's IP.                                                                                                     |
...
Ultraglass allows you to customise the colours and fonts used in the web UI.

| Parameter                      | Type   | Default Value | Description                                                                                                      |
| :----------------------------- | :----- | :------------ | :--------------------------------------------------------------------------------------------------------------- |
| `web.theme.default_color_mode` | String |               | Set Ultraglass's default colour mode. By default, the user's system preference is used. Must be `light` or `dark` |
...
To override Ultraglass's primary colour, override its mapped intrinsic colour. For example, to override the default primary colour to <Color hex="#00ffff" noText /> cyan:
...
Ultraglass's fonts are loaded from [Google Fonts](https://fonts.google.com/). Any Google font name may be specified to override the default fonts.
