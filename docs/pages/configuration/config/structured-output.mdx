...
For devices with structured traceroute support (MikroTik RouterOS and Huawei VRP), hyperglass can enhance the output with IP enrichment data including ASN information, organization names, country codes, and IXP detection using offline data from BGP.tools and PeeringDB.
...
| `structured.ip_enrichment.enabled`          | Boolean         | false         | Enable IP enrichment using offline BGP.tools and PeeringDB data for structured outputs.                                              |
...
| `structured.ip_enrichment.enrich_next_hop`  | Boolean         | false         | Enrich BGP next-hop IP addresses with ASN and organization information.                                                              |
| `structured.ip_enrichment.enrich_traceroute`| Boolean         | true          | Enrich traceroute hop IP addresses with ASN, organization, and IXP information.                                                      |
...
<Callout type="info" emoji="ℹ️">
    **IP Enrichment Requirements**
    ...
    IP enrichment is currently supported for:
    - **Traceroute**: MikroTik RouterOS and Huawei VRP platforms
    - **BGP Routes**: When `enrich_next_hop` is enabled
...
#### Enable Full IP Enrichment (Traceroute + BGP Next-Hop)

```yaml filename="config.yaml" copy {2-6}
structured:
    ip_enrichment:
        enabled: true
        enrich_traceroute: true
...
structured:
    ...
    ip_enrichment:
        enabled: true
        enrich_traceroute: true
        enrich_next_hop: false  # Only enrich traceroute, not BGP
```
...
structured:
    ip_enrichment:
        enabled: false  # Traceroute will show basic hop info without ASN/org data
```
...
<Callout type="warning" emoji="⚠️">
    **IP Enrichment Dependency**
    ...
    For the full structured traceroute experience with rich metadata, `ip_enrichment.enabled: true` is required.
</Callout>
